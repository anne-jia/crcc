        ...
        <div class="crcc-process-palette-container">
            <div class="crcc-control_header">
                ...
                <el-button-group key="file-control">
                    ...
                    <el-button type="primary" ref="saveDiagram" @click="downloadProcessAsBpmn()">
...
import checkDefinition from "./rules/DefinitionChecker";
...
export default {
    ...
    props: {
        ...
        events: {
            type: Array,
            default: () => ['element.click','opening-message','opening-participant','saved-participant','saved-message'],
        },
    },
    watch: {
    ...
    methods: {
        ...
        initModelListeners() {
            ...
            this.events.forEach((event) => {
                  if(event.includes('.')){
                    ...
                    console.log(this.$workflowEventBus,)
                    this.$workflowEventBus.$on(event, function (e) {
                        that.$emit(event,e)
                      })
                    console.log(event);
        ...
        saveDiagram(done) {
            ...
            this.clearBPMNEdageBounds(this.bpmnModeler._definitions);
            let defintionErrors = checkDefinition(this.bpmnModeler._definitions);
        ...
        savedMessage(result) {
            if (result) {
                this.$workflowEventBus.$emit("saved-message", result);
            }
        },
        ...
        savedParticipant(result) {
            if (result) {
                this.$workflowEventBus.$emit("saved-participant", result);
            }
        }
    },
};
